...
红黑树版本目前只有调度器，感兴趣的读者可参考链表版本内核自行移植其他功能。
...
目前红黑树版本的开发搁置中，该版本只有基本的调度功能，与数表版本的调度层实现功能是一样的。

整体来看，红黑树版本性能不如链表版本。

为了应对大量任务的实时场景，同时发挥红黑树的优势，笔者考虑是否使用EDF等动态优先级算法重新设计一个调度器。

笔者之前对红黑树和调度器进行了一些改动，但效果都不尽人意，所以先保留该版本的内核调度器和红黑树，等有新思路了再重新设计调度算法。

红黑树版本与链表版本只是数据结构存在差别，如果读者对红黑树版本内核感兴趣，只需要将链表版本中对链表的操作替换为红黑树的操作即可完成移植。
