...
Sparrow RTOS共有三个版本：数表版本、链表版本、红黑树版本。

三个版本中支持的编译器和平台正在维护更新中，目前默认支持的架构是arm cm3，默认支持的编译器为gcc。

数表版本支持32个以下的任务，也不支持同优先级，该版本支持keil、gcc、IAR多种编译器。

链表版本支持同优先级，对支持的任务数量没有限制，该版本接口部分可参考数表版本进行修改。

红黑树版本适用于大量任务频繁搜索、插入和删除操作的场景。该版本目前只有调度器，调度算法的开发暂时搁置。
...
目前红黑树版本的开发还在思考中，该版本只有基本的调度功能，与数表版本的调度层实现功能是一样的。

说实话，多优先级队列对于小型RTOS是最合适的。可能红黑树的优势要在大型RTOS中才能体现出来，但大型RTOS（数百万上千万行代码）开源的极少，笔者对这些调度算法也所知甚少，也难以想象如何使用红黑树设计一个合理的调度算法。

笔者之前对红黑树和调度器进行了一些改动，但效果都不尽人意，所以先保留该版本的内核调度器和红黑树，等有新思路了再重新设计调度算法。
